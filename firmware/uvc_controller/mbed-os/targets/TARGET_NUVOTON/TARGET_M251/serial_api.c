...
void serial_set_flow_control(serial_t *obj, FlowControl type, PinName rxflow, PinName txflow)
{
    ...
    if (rxflow != NC) {
        ...
        // NOTE: Added in M480/M2351/M251. Before configuring RTSACTLV, disable TX/RX.
        ...
        // NOTE: Added in M480/M2351/M251. After configuring RTSACTLV, re-enable TX/RX.
        uart_base->FUNCSEL &= ~UART_FUNCSEL_TXRXDIS_Msk;
    ...
    if (txflow != NC) {
        ...
        // NOTE: Added in M480/M2351/M251. Before configuring CTSACTLV, disable TX/RX.
        uart_base->FUNCSEL |= UART_FUNCSEL_TXRXDIS_Msk;
        while (uart_base->FIFOSTS & UART_FIFOSTS_TXRXACT_Msk);
        // nCTS pin input is low level active
        uart_base->MODEMSTS |= UART_MODEMSTS_CTSACTLV_Msk;
        // NOTE: Added in M480/M2351/M251. After configuring CTSACTLV, re-enable TX/RX.
        uart_base->FUNCSEL &= ~UART_FUNCSEL_TXRXDIS_Msk;
    ...
    if (obj->serial.dma_usage_rx == DMA_USAGE_NEVER) {
        ...
        PDMA_SetTransferAddr(pdma_base,
                             ...
                             (uint32_t) rx,     // NOTE:
                                                ...
                                                // M251: Start of destination address
