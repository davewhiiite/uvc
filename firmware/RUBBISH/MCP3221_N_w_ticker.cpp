...
//Create instance
MCP3221_N::MCP3221_N(PinName sda, PinName scl, float supplyVoltage, uint8_t N) : i2c(sda, scl), _supplyVoltage(supplyVoltage), _enn(N)
{
}
...
void MCP3221_N::read(float &returnValue)
{
...
//I2C.STOP
    ...
    for (index=0x00; index<_enn; index++)
    {
        /* ADC start conv */    
        i2c.start();
    ...
    *returnValue = (_supplyVoltage/4096) * avg_sample;
}
